...
choice_cmpds <- data.frame(
  "name" = c("PFOA", "TCDD", "DDT", "6-OH-BDE47"), 
  "molecular_weight" = c(414.07, 322.0, 354.5, 501.79),
  "x_log_p" = c(4.9, 6.4, 6.9, 5.9)
)
...
p +
  ...
  theme_classic() +
    geom_rect(aes(xmin = 0, xmax = 500, ymin = 0, ymax = 5), 
            alpha = 0, color = "black") +
  geom_point(data = choice_cmpds, 
             aes(x = molecular_weight, 
                 y = x_log_p, 
                 label = name), colour = "black") +
  ggrepel::geom_text_repel(data = choice_cmpds, 
                            aes( x = molecular_weight, 
                                 y = x_log_p, 
                                 label = name))
